...
async def interface_upscale_image(encoded_image, size=2, upscaler="SwinIR 4x"):
    ...
    image_data = ""
    async with aiohttp.ClientSession() as session:
        async with session.post(host + "/sdapi/v1/extra-single-image", json=request) as resp:
            r = await resp.json()
            ...
            image_data = "data:image/png;base64," + r["image"]
...
async def interface_txt2img(prompt: str = "", seed: int = -1, quantity: int = 1, negative_prompt: str = "", simulate_nai: bool = True, host: str = None):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        # With request["send_images"] = True, the HTTP API will always return the full images, base64-encoded
        # under response["images"]
        async with session.post(host + "/sdapi/v1/txt2img", json=request) as resp:
            r = await resp.json()
...
async def interface_img2img(prompt: str = "", seed: int = -1, quantity: int = 1, negative_prompt: str = "", simulate_nai: bool = True, img2img_image_data: str = "", denoising_strength: float = 0.6, host: str = None):
    ...
    dependency_data = {}
    fn_index, dependency_data = gradio_mapper.find_dependency_data_to_component(component_id=target, js_string="submit_img2img")
